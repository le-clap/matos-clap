...
export function DashboardPage() {
  ...
  const maintenanceCount = items.data?.filter((i) => i.availability === 'maintenance').length ?? 0;
  const overdueCount = activeLoans.data?.items.filter(isOverdue).length ?? 0;
  ...
  return (
    <div>
      ...
      {overdueCount > 0 && (
        <div className="mt-4 flex items-center gap-3 rounded-[var(--radius-card)] border border-danger/30 bg-danger-bg px-4 py-3 text-sm">
          ...
          <span className="text-brand-200">
            <strong>{overdueCount}</strong> prêt(s) en cours ont dépassé leur date de retour prévue.
          </span>
          <Link
            to="/admin/loans"
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        <Card>
          <ListHeader title="Demandes à traiter" to="/admin/requests" icon={ClipboardList} />
              ...
              pending.data.items.map((req) => (
                ...
                >
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium">{req.borrower.name}</p>
          ...
          <CardBody className="flex flex-col gap-1 p-2">
            {recentLoans.isLoading ? (
              ...
              recentLoans.data.items.map((loan) => (
                ...
                >
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium">{loan.borrower.name}</p>
                    <p className="truncate text-xs text-content-faint">
                      {loan.loaned_items.length} article(s) · {formatDateShort(loan.start_date)}
                    </p>
                  </div>
                  {isOverdue(loan) ? (
                ...
                >
                  ...
                  <Td>
                    <div className="flex items-center gap-2">
                      <span className="text-content-muted">{formatDateShort(o.endDate)}</span>
                      {o.overdue && (
                        <Badge tone="danger" dot>
                          En retard
                        </Badge>
      ...
      <CardBody className="flex items-center gap-4">
        ...
        <div className="min-w-0">
          {loading ? (
            ...
            <p className="text-2xl font-bold tabular-nums leading-none">{value ?? 0}</p>
          )}
...
function ListHeader({ title, to, icon: Icon }: { title: string; to: string; icon: LucideIcon }) {
  return (
    <div className="flex items-center justify-between border-b border-border px-4 py-3.5">
      ...
      <Link to={to} className="text-sm text-content-muted transition-colors hover:text-content">
        Tout voir
      </Link>
    </div>
  );
...
function Empty({ label }: { label: string }) {
  return <p className="px-3 py-8 text-center text-sm text-content-faint">{label}</p>;
}
