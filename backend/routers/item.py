...
def create_item(item: Item, session: Session = Depends(get_session)):
    if not session.get(Catalog, item.catalog_id):
        raise HTTPException(status_code=404, detail=f"Catalog entry with id {item.catalog_id} not found")

    if not session.get(Condition, item.condition_id):
        raise HTTPException(status_code=404, detail=f"Condition id {item.condition_id} not found")

    if not session.get(Availability, item.availability_id):
        raise HTTPException(status_code=404, detail=f"Availability id {item.availability_id} not found")
