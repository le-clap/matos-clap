...
def get_availability_search(name: str = "Available", session: Session = Depends(get_session)):
    availability = session.exec(select(Availability).where(Availability.name == name)).first()

    if not availability:
        raise HTTPException(status_code=404, detail=f"Availability class {name} not found")
