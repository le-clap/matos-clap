...
from models.models import Availability, Catalog, Category, Condition, Item, User
...
def seed_data():
    with Session(engine) as session:
        ...
        assert cond_new.id is not None
        assert cond_good.id is not None
        assert cond_bad.id is not None
        assert avail_ok.id is not None
        assert avail_loaned.id is not None
        assert avail_maint.id is not None
        assert cat_camera.id is not None
        assert cat_light.id is not None
